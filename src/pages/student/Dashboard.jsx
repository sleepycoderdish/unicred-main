...
// Stub — replace with full dashboard once backend APIs are ready.
...
export default function StudentDashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16, background: 'var(--bg-base)', color: 'var(--text-primary)', fontFamily: 'var(--font-sans)' }}>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 700 }}>Student Dashboard</h1>
      <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
        Logged in as <strong>{user?.userId}</strong> | Role: <strong>{user?.role}</strong>
      </p>
